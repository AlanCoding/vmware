...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _populate_from_source(self, source_data, using_current_cache):
        ...
        for vm_obj in objects:
            for vm_obj_property in vm_obj.propSet:
                ...
                if vm_obj.obj.config.uuid not in seen_uuids:
                    seen_uuids.add(vm_obj.obj.config.uuid)

                    host_properties = self.get_host_properties(vm_obj)

                    current_host = self._get_hostname(host_properties, hostnames, strict=strict)
                    ...
                    self._populate_host_properties(host_properties, current_host)

                    # Use constructed if applicable
                    # Composed variables
                    self._set_composite_vars(self.get_option('compose'), host_properties, current_host, strict=strict)
                    # Complex groups based on jinja2 conditionals, hosts that meet the conditional are added to group
                    self._add_host_to_composed_groups(self.get_option('groups'), host_properties, current_host, strict=strict)
                    # Create groups based on variable values and add the corresponding hosts to it
                    self._add_host_to_keyed_groups(self.get_option('keyed_groups'), host_properties, current_host, strict=strict)
    ...
    def get_host_properties(self, vm_obj):
        props = {}
        # obtain properties that will become hostvars
        ...
        for vm_prop in vm_properties:
            if vm_prop == 'customValue':
                for cust_value in vm_obj.obj.customValue:
                    key = [y.name for y in field_mgr if y.key == cust_value.key][0]
                    props[key] = cust_value.value
            else:
                vm_value = self.pyv._get_object_prop(vm_obj.obj, vm_prop.split("."))
                props[vm_prop] = vm_value
        return props
